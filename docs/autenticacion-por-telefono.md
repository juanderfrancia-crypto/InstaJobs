...
### Paso 5 — Selección de rol (`RoleScreen`) — solo usuarios nuevos

El nuevo usuario elige cómo quiere usar la app:

| Opción | Rol asignado | Descripción |
|---|---|---|
| "Necesito un trabajador" | `client` | Puede publicar trabajos y contratar |
| "Ofrezco mis servicios" | `worker` | Puede postularse a trabajos disponibles |

> El rol se puede cambiar después desde el perfil.

---

### Paso 6 — Completar perfil (`OnboardingScreen`) — solo usuarios nuevos

El usuario completa su perfil inicial. Los campos varían según el rol elegido:

#### Campos comunes (cliente y trabajador)
| Campo | Obligatorio | Descripción |
|---|---|---|
| Nombre completo | Sí | Nombre que verán otros usuarios |
| Municipio o ciudad | Sí | Usado para mostrar trabajos/trabajadores cercanos |
| Foto de perfil | No | Se puede agregar o cambiar después |

#### Campos adicionales (solo trabajadores)
| Campo | Obligatorio | Descripción |
|---|---|---|
| WhatsApp de contacto | No | Número para que clientes contacten directamente |
| Oficios que ofrece | Sí | Mínimo 1 categoría (plomería, electricidad, etc.) |
| Años de experiencia | No | Número entero |
| Sobre mí (bio) | No | Descripción libre, máx. ~400 caracteres |

Al finalizar, el usuario debe **aceptar los Términos y Política de Privacidad** para poder continuar. El botón "Entrar a InstaJobs" guarda el perfil en Supabase y redirige al `HomeScreen`.
